...
with open(thermo_root+"/Compounds_GroupFormation_eQuilibrator_Comparison.txt") as fh:
    ...
    for line in fh.readlines():
        ...
        if(gf_en=="nan" or "10000000.0" in gf_en or eq_en=="nan"):
            continue

        (gf_dg,gf_dge) = gf_en.split('|')
#        if(float(gf_dg)!=0):
#            print(float(gf_dge)/abs(float(gf_dg)),gf_dg,gf_dge)
        (eq_dg,eq_dge) = eq_en.split('|')

        if(float(eq_dg)!=0):
            print(float(eq_dge)/abs(float(eq_dg)),eq_dg,eq_dge)

        compounds_energies[cpd]={'gf':[float(gf_dg),float(gf_dge)],'eq':[float(eq_dg),float(eq_dge)]}

categories=rxn_type_cpds.keys()
