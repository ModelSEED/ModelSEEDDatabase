...
| `TECRDB_COMPARISON` | `Biochemistry/Thermodynamics/SourceGrading/tecrdb_comparison.csv` |

## The TECRDB comparison is generated here

`tecrdb_comparison.csv` is the ground truth — the anchor every
...
filesystem. It is now generated by `build_tecrdb_comparison.py` in this
directory.

    # anchors, then grades
    python3 build_tecrdb_comparison.py          # needs rdkit + pandas
...
`build_comparison.py` in his `dgpredictor_tecrdb` tree; only the paths were
...
`TECRDB_SOURCE`, `TECRDB_COMPARISON_OUT`.
