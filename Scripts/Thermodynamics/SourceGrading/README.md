...
| `openTECR_COMPARISON` | `Biochemistry/Thermodynamics/SourceGrading/opentecr_comparison.csv` |

## The openTECR comparison is generated here

`opentecr_comparison.csv` is the ground truth — the anchor every
accuracy number is measured against. It used to be an input nobody could
produce: three scripts read it, none wrote it, and it came from a third
filesystem. It is now generated by `build_opentecr_comparison.py` in this
directory.

    # anchors, then grades
    python3 build_opentecr_comparison.py          # needs rdkit + pandas
    python3 grade_thermo_sources.py --cv        # --cv for leak-free validation

The matching logic is **Cooper Taylor's**, ported unchanged from
`build_comparison.py` in his `dgpredictor_opentecr` tree; only the paths were
rewritten to derive from this repository. Two inputs necessarily differ from
his run — the live `Biochemistry/` rather than his pinned June snapshot, and
our `tecrdb_v11` because his `TECRDB.csv` is not world-readable — so the
anchor set is **797 stereo-exact rather than his 802**. 789 are shared and
their measured energies agree to 0.0000 kJ, and every published figure moved
by under one point when the sets were swapped.

Paths are overridable: `MSDB_ROOT`, `MSDB_BIOCHEM`, `DGPREDICTOR_JSON`,
`OPENTECR_SOURCE`, `openTECR_COMPARISON_OUT`.

Run `grade_thermo_sources.py` with `--cv`. Without it the validation table is
in-sample against the very anchors the calibration was fitted on, which is how
a 95%/45% pair reached a manuscript draft that the leak-free numbers put at
92.5% and no quotable bronze rate.
...
* eQuilibrator is *fitted* on openTECR and the anchor *is* openTECR, so its margin
