...
my @compartments=@{$doc->getElementsByTagName('sbml:listOfCompartments')}[0]->getChildrenByTagName('*');
...
my @compounds=@{$doc->getElementsByTagName('sbml:listOfSpecies')}[0]->getChildrenByTagName('*');
...
my @notes=(); #CM
my @anns=(); #LN
my @reactions=@{$doc->getElementsByTagName('sbml:listOfReactions')}[0]->getChildrenByTagName('*');
...
foreach my $c (@reactions){
    ...
    $reacts="";
    if(scalar(@{$c->getChildrenByTagName('sbml:listOfReactants')})>0){
	my @reactants_left=@{$c->getChildrenByTagName('sbml:listOfReactants')}[0]->getChildrenByTagName('*');
    ...
    $prods="";
    if(scalar(@{$c->getChildrenByTagName('sbml:listOfProducts')})>0){
	my @products_right=@{$c->getChildrenByTagName('sbml:listOfProducts')}[0]->getChildrenByTagName('*');
