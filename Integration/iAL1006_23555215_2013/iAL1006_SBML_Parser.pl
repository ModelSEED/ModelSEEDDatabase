...
print FH "ID\tName\tCompartment\tCharge\tboundary Condition\tInChI\n";
foreach my $c (@compounds){
    ...
    my @refs = $c->getElementsByTagName('in:inchi');
    my $InChI="";
    if(scalar(@refs)>0){
	$InChI=$refs[0]->firstChild()->toString();
    }
    print FH $id,"\t",$name,"\t",$cpt,"\t",$charge,"\t",$bC,"\t",$InChI,"\n";
}
close(FH);
