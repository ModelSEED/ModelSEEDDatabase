...
**dGpredictor retrained on ModelSEED.** dGpredictor (Wang et al. 2021, *PLOS Comput Biol*, [10.1371/journal.pcbi.1009448](https://doi.org/10.1371/journal.pcbi.1009448)) decomposes each molecule into radius-1 and radius-2 atom-centred fragments and fits a Bayesian ridge regression over the fragment counts. The model deployed here is retrained rather than the published one: the shipped fragment vocabulary was written by an older RDKit and no longer round-trips, so it is rebuilt from current-RDKit output with training and prediction sharing one decomposition routine; and the retrain is fitted to the de-duplicated TECRDB table (4,456 measurements), the same table eQuilibrator's component contribution is fitted to, so the two methods are compared on the same measurements. Fitted rows and active features: [TBD]. Because the retrain includes formation-energy pseudo-reactions, the model reports per-compound ΔfG′ alongside per-reaction ΔrG′: 29,617 reactions and 30,185 compounds, of which 2,291 reactions and 3,020 compounds carry neither a Group Contribution nor an eQuilibrator estimate. Every prediction is installed with its own uncertainty and no coverage floor: the uncertainties are wide (median 71.2 kJ/mol, 28.2% above 100 kJ/mol) but calibrated against eQuilibrator at every coverage stratum, so consumers filter on uncertainty rather than on fragment coverage. Full account in Methods draft `drafts/methods_multi_source_thermodynamics.md`.

**Archived 2020 dGpredictor baseline.** The ΔrG′ values carried since 2023 came from the published KEGG-trained dGpredictor, mapped onto ModelSEED reactions through their KEGG aliases. A stale-variable defect in the staging step — the alias-extraction loop never reset its holding variable between reactions — left every reaction without a KEGG alias holding the identifier of the nearest preceding reaction that had one: 17,271 of 27,715 staged records (62%). The contaminated subset shows no correlation with the other estimators (|r| < 0.01) while the correctly-mapped subset agrees as expected (r = 0.61 vs eQuilibrator, 0.74 vs Group Contribution). Those records are retained only as an archived baseline and are not a live source; the Results head-to-head uses the retrained arm, which is immune by construction (keyed by ModelSEED reaction id, no alias mediation).
